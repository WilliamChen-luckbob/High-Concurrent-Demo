...
public class JwtUtils {
    ...
    private static final String DEFAULT_BASE64_ENCODED_SECRETKEY = "dGhpcyBpcyBhIGp3dCB0b2tlbiBhdXRob3JpemVkIGJ5IGluZmlhaSwgY29kZSBieSB3aWxsaWFt";
    private static final SignatureAlgorithm DEFAULT_SIGNATURE_ALGORITHM = SignatureAlgorithm.HS256;

//    public JwtUtils() {
//        this(DEFAULT_BASE64_ENCODED_SECRETKEY, DEFAULT_SIGNATURE_ALGORITHM);
//    }
//
...
//    }
     ...
     * @return
     */
    public static String encode(String iss, long ttlMillis, Map<String, Object> claims, String customizedUUID) {
        ...
        JwtBuilder builder = Jwts.builder()
                .setClaims(claims)
                .setId(StringUtils.isEmpty(customizedUUID) ? UUID.randomUUID().toString() : customizedUUID)//2. 这个是JWT的唯一标识，一般设置成唯一的，这个方法可以生成唯一标识
                .setIssuedAt(new Date(nowMillis))//1. 这个地方就是以毫秒为单位，换算当前系统时间生成的iat
                .setSubject(iss)//3. 签发人，也就是JWT是给谁的（逻辑上一般都是username或者userId）
                .signWith(DEFAULT_SIGNATURE_ALGORITHM, DEFAULT_BASE64_ENCODED_SECRETKEY);//这个地方是生成jwt使用的算法和秘钥
        if (ttlMillis >= 0) {
    ...
    public static Claims decode(String jwtToken) {
        ...
        return Jwts.parser()
                // 设置签名的秘钥
                .setSigningKey(DEFAULT_BASE64_ENCODED_SECRETKEY)
                // 设置需要解析的 jwt
...
//            case HS256:
        algorithm = Algorithm.HMAC256(Base64.decodeBase64(DEFAULT_BASE64_ENCODED_SECRETKEY));
//                break;
//            default:
...
//        以tom作为秘钥，以HS256加密
        ...
        String jwtToken = encode("tom", 30000, map, null);
