...
public class RedisUtils {
    ...
    public void set(String key, Object value, long expire, TimeUnit timeUnit) {
        if (expire != RedisExpireTime.NEVER_EXPIRE.getExpireTime()) {
            valueOperations.set(key, toJson(value), expire, timeUnit);
        } else {
    ...
    public Map getMapAndExpire(String mapName, RedisExpireTime expireTime) {
        Map map = getMap(mapName);
        if (CollectionUtil.isNotEmpty(map) && expireTime != RedisExpireTime.NEVER_EXPIRE) {
            redisTemplate.expire(mapName, expireTime.getExpireTime(), TimeUnit.SECONDS);
        }
        return map;
    }
    ...
    public void putSet(String setName, Set set, RedisExpireTime redisExpireTime) {
        ...
        if (redisExpireTime != RedisExpireTime.NEVER_EXPIRE) {
            redisTemplate.expire(setName, redisExpireTime.getExpireTime(), TimeUnit.SECONDS);
        }
    }
    ...
    public void expire(String key, RedisExpireTime redisExpireTime) {
        redisTemplate.expire(key, redisExpireTime.getExpireTime(), TimeUnit.SECONDS);
    }
